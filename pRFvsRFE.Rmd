...
for (kot in 1:runs) {
  ...
  p.test<-pRF(response=y,mtry=mtry,
              predictors=x,n.perms=500,ntree=ntree,seed=seed,
...
print(paste('Based on',runs,'runs with 300 permtations each'))
...
plot(y,rfepred); title(paste0('pRF selected prediction\n',paste(colnames(x)[rfevars], collapse=' ')))
plot(y,prfpred); title(paste0('pRF selected prediction\n',paste(colnames(x)[prfvars], collapse=' ')))
